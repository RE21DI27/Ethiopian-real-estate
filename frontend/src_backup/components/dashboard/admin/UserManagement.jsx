...
import { Search, UserCheck, UserX, Trash2, Eye, Shield, CheckCircle, XCircle, AlertCircle } from 'lucide-react'
...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const url = `${API_URL}/api/admin/users?search=${searchTerm}&status=${filterStatus !== 'all' ? filterStatus : ''}`
  ...
  const handleSuspendUser = async (userId) => {
    if (!window.confirm('Are you sure you want to suspend this user? They will not be able to access the platform.')) return
  ...
  // Action 3: Activate User (for suspended users)
  ...
  const getStatusBadge = (statusDisplay) => {
  ...
  // Stats calculations (excluding admin)
  const verifiedCount = users.filter(u => u.status_display === 'Verified').length
  const unverifiedCount = users.filter(u => u.status_display !== 'Verified' && u.status !== 'suspended').length
          ...
          <div className="p-6">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                {getStatusBadge(selectedUser.status_display)}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="border-b pb-2">
                ...
                <p className="font-medium capitalize">{selectedUser.role_type}</p>
              </div>
              <div className="border-b pb-2">
                <p className="text-sm text-gray-500">Registered</p>
                <p className="font-medium">{selectedUser.registered_date}</p>
              </div>
              <div className="border-b pb-2">
                  ...
                  <tr key={user.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {/* Icon 2: Verify User - CheckCircle (only for unverified users) */}
                        {!user.is_verified && user.status !== 'suspended' && (
                          <button 
                            ...
                            title="Verify User"
                          >
                            <UserCheck className="w-4 h-4 text-green-500 group-hover:text-green-700" />
                          </button>
                        )}
                        
                        {/* Icon 3: Suspend/Activate based on status */}
                        {user.status === 'suspended' ? (
                          <button 
                            ...
                            title="Activate User"
                          >
                            <Shield className="w-4 h-4 text-blue-500 group-hover:text-blue-700" />
                          </button>
                        ) : (
                          <button 
                            ...
                            title="Suspend User"
                          >
                            <UserX className="w-4 h-4 text-red-500 group-hover:text-red-700" />
                          </button>
                        )}
                        
                        {/* Icon 4: Delete User - Trash */}
                        <button 
