...
const ActivationPage = ({ defaultRole, onActivationSuccess }) => {
  ...
  const uploadLandlordPhotos = async (files) => {
    const uploadedUrls = []
  ...
  const submitSellerActivation = async () => {
    ...
    if (sellerForm.business_license) {
      businessLicenseUrl = await uploadFile(sellerForm.business_license, 'license')
    }
    if (sellerForm.ownership_document) {
      ownershipDocUrl = await uploadFile(sellerForm.ownership_document, 'ownership')
    }
    if (sellerForm.government_id) {
      governmentIdUrl = await uploadFile(sellerForm.government_id, 'id')
    }
  ...
  const submitLandlordActivation = async () => {
    ...
    if (landlordForm.property_title_deed) {
      titleDeedUrl = await uploadFile(landlordForm.property_title_deed, 'deed')
    }
    if (landlordForm.property_tax_clearance) {
      taxClearanceUrl = await uploadFile(landlordForm.property_tax_clearance, 'tax')
    }
    if (landlordForm.government_id) {
      governmentIdUrl = await uploadFile(landlordForm.government_id, 'id')
    }
