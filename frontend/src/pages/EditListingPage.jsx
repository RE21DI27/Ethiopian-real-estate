...
import { 
  ...
  Star, Calendar, Phone, Mail, Building, Calendar as CalendarIcon
} from 'lucide-react'
      ...
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6">
          <div className="relative h-64 bg-gray-200 rounded-xl mb-4">
            {existingImages[coverImageIndex] && (
              <img src={`${API_URL}${existingImages[coverImageIndex].url}`} className="w-full h-full object-cover rounded-xl" />
            )}
          </div>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
            <p><Phone className="w-4 h-4 inline mr-2" />{formData.phone_number}</p>
            <p><Mail className="w-4 h-4 inline mr-2" />{formData.email}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <main className={`transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-20'}`}>
        ...
        <div className="p-6 max-w-5xl mx-auto">
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-2xl shadow-lg border p-8">
                ...
                <div className="space-y-6">
                  <div><label className="block text-sm font-medium mb-2">Property Title *</label><input name="title" value={formData.title} onChange={handleChange} className="w-full p-4 border rounded-xl" /></div>
                  <div className="grid grid-cols-2 gap-6">
                    <div><label className="block text-sm font-medium mb-2">Listing Type</label>
                    ...
                    <div><label className="block text-sm font-medium mb-2">Property Type</label>
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div><label className="block text-sm font-medium mb-2">Price (ETB) *</label><input name="price" type="number" value={formData.price} onChange={handleChange} className="w-full p-4 border rounded-xl" /></div>
            ...
            {/* Step 3: Address Details (Sub City & Kebele) */}
            {step === 3 && (
            ...
            {step === 5 && (
              <motion.div key="step5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-2xl shadow-lg border p-8">
                ...
                <div className="space-y-6">
                  ...
                  {/* Contact Information */}
                  <div className="grid grid-cols-2 gap-6">
                    <div><label className="block text-sm font-medium mb-2 flex items-center gap-2"><Phone className="w-4 h-4" />Phone Number</label><input name="phone_number" value={formData.phone_number} onChange={handleChange} className="w-full p-4 border rounded-xl" placeholder="+251 911 111 111" /></div>
                    <div><label className="block text-sm font-medium mb-2 flex items-center gap-2"><Mail className="w-4 h-4" />Email Address</label><input name="email" type="email" value={formData.email} onChange={handleChange} className="w-full p-4 border rounded-xl" placeholder="contact@property.com" /></div>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <div className="flex justify-between mt-8">
            ...
            <div className="flex gap-3 ml-auto">
              ...
              {step === 5 && <button onClick={() => handleUpdate(false)} disabled={loading} className="px-6 py-3 border-2 rounded-xl font-semibold flex items-center gap-2">{loading ? <Loader className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}Save Changes</button>}
              {step === 5 && isDraft && <button onClick={() => handleUpdate(true)} disabled={loading} className="px-6 py-3 bg-green-600 text-white rounded-xl font-semibold flex items-center gap-2"><CheckCircle className="w-4 h-4" />Publish</button>}
