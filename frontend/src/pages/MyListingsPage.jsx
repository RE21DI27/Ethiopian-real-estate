...
const MyListingsPage = () => {
  ...
  const fetchMyListings = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/listings/my-listings?include_drafts=${showDrafts}`, {
  ...
  const handleView = (listingId) => {
    // Try to navigate to property detail page
        ...
        <div className="p-6">
          <div className="flex justify-end mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={showDrafts} onChange={(e) => setShowDrafts(e.target.checked)} className="w-4 h-4 text-blue-600 rounded" />
                ...
                <div key={listing.id} className="bg-white rounded-2xl shadow-sm border overflow-hidden hover:shadow-lg transition">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleView(listing.id)} className="flex-1 px-3 py-2 border rounded-lg text-sm font-medium hover:bg-gray-50 flex items-center justify-center gap-1">
                        <Eye className="w-4 h-4" />View
                      </button>
                      <button onClick={() => handleEdit(listing)} className="flex-1 px-3 py-2 border border-blue-200 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-50 flex items-center justify-center gap-1">
